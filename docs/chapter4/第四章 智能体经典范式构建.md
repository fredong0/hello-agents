...
1. 本章介绍了三种经典的智能体范式:`ReAct(1)`、`Plan-and-Solve(2)` 和 `Reflection(3)`。请分析:

   - 这三种范式在"思考"与"行动"的组织方式上有什么本质区别？<br>
     (1)是一种最基础的思考行动思考行动的流程
     (2)先对问题套上一个整体规划，然后带着规划进prompt，依次思考行动
     (3)reflect是事后的，对最终输出的一个整体评价，他不是思考下一步做什么，而是判断已经输出的东西做得怎么样。他的目的主要是质量的把关与提升。

   - 如果要设计一个"智能家居控制助手"（需要控制灯光、空调、窗帘等多个设备，并根据用户习惯自动调节），你会选择哪种范式作为基础架构？为什么？<br>
 	使用(2)对用户需求和场景做一个整体的规划（比如年轻人要打游戏他需要一个怎样的场景，老年人起床晨练需要一个怎样的场景，中登下班回来需要一个怎样的场景），
 	然后生成各自的指令完成他

   - 是否可以将这三种范式进行组合使用？若可以，请尝试设计一个混合范式的智能体架构，并说明其适用场景。<br>
      可以。这需要在提示詞里面对不同的输出进行格式的规范。当plan出来需要action的时候做输出格式是什么，需要推理的时候输出格式是什么。reflect的介入也可以存在于多个地方，比如
      一次action完之后的reflect，对输出最终结果的reflect等

2. 在4.2节的 `ReAct` 实现中，我们使用了正则表达式来解析大语言模型的输出（如 `Thought` 和 `Action`）。请思考:

   - 当前的解析方法存在哪些潜在的脆弱性？在什么情况下可能会失败？<br>
                弱的模型可能无法完全遵循格式要求去生成
   - 除了正则表达式，还有哪些更鲁棒的输出解析方案？<br>
	输出指定字段的json？然后从json字段中读取?
...
3. 工具调用是现代智能体的核心能力之一。基于4.2.2节的 `ToolExecutor` 设计，请完成以下扩展实践:
   ...
   - 设计并实现一个"工具选择失败"的处理机制:当智能体多次调用错误的工具或提供错误的参数时，系统应该如何引导它纠正？
	工具的desc里面设计一简单一详细的desc，当调用出错时系统给出这个tool详细版本的desc。如果两个工具本身有一定的相似性，易混淆。desc中需要重点指出其二者区别。
   - 思考:如果可调用工具的数量增加到$50$个甚至$100$个，当前的工具描述方式是否还能有效工作？在可调用工具数量随业务需求显著增加时，从工程角度如何优化工具的组织和检索机制？
	100多条desc增大了上下文的同时，也带来了很多冗余输入。把工具一个个规整到工具箱内显然有必要。比如根据部门职能，工作组划分。根据功能划分，根据复杂度划分。或者最简单的人工激活tool直接指定。

4. `Plan-and-Solve` 范式将任务分解为"规划"和"执行"两个阶段。请深入分析:

   - 在4.3节的实现中，规划阶段生成的计划是"静态"的（一次性生成，不可修改）。如果在执行过程中发现某个步骤无法完成或结果不符合预期，应该如何设计一个"动态重规划"机制？<br>
	大模型应该要产生执行成功/失败的分支，失败时将失败的情况记录到history然后丢给plan重新制定计划。如果无法直接界定为失败，可以采用reflect的方案对执行/思考结果进行审视，必要时重新制定整个计划或者更改当前步骤
   - 对比 `Plan-and-Solve` 与 `ReAct`:在处理"预订一次从北京到上海的商务旅行（包括机票、酒店、租车）"这样的任务时，哪种范式更合适？为什么？<br>
能做plan肯定是plan更好吧。
   - 尝试设计一个"分层规划"系统:先生成高层次的抽象计划，然后针对每个高层步骤再生成详细的子计划。这种设计有什么优势？

5. `Reflection` 机制通过"执行-反思-优化"循环来提升输出质量。请思考:

   - 在4.4节的代码生成案例中，不同阶段使用的是同一个模型。如果使用两个不同的模型（例如，用一个更强大的模型来做反思，用一个更快的模型来做执行），会带来什么影响？<br>
      不同能力的模型负责agent中不同的任务是很常见的设计，比如更强的模型用于规划plan，弱的模型拿到细化的布置进行实际的简单执行，这节省了贵价模型token的消耗。
   - `Reflection` 机制的终止条件是"反馈中包含<strong>无需改进</strong>"或"达到最大迭代次数"。这种设计是否合理？能否设计一个更智能的终止条件？<br>
      暂时没想到
   - 假设你要搭建一个"学术论文写作助手"，它能够生成初稿并不断优化论文内容。请设计一个多维度的Reflection机制，从段落逻辑性、方法创新性、语言表达、引用规范等多个角度进行反思和改进。<br>
      没想好

6. 提示词工程是影响智能体最终效果的关键技术。本章展示了多个精心设计的提示词模板。请分析:

   - 对比4.2.3节的 `ReAct` 提示词和4.3.2节的 `Plan-and-Solve` 提示词，它们显然存在结构设计上的明显不同，这些差异是如何服务于各自范式的核心逻辑的？<br>
react强调了工具的使用以及输出的格式规范，以便于agent的解析和调用工具，即用于act的部分。而plan-solve首先区分了plan和solve两个角色，即各自为政。
